...
export class FlowerService {
  ...
  public getById(id: number): Flower {
    return this.flowerList.filter((flower) => flower.id == id)[0];
  }

  public update(flowerId: number, updateFlower: Flower) {
    const flower: Flower = this.flowerList.filter(
      (flower) => flower.id == flowerId
    )[0];

    flower.imageUrl = updateFlower.imageUrl;
    flower.name = updateFlower.name;
    flower.price = updateFlower.price;
    flower.quantity = updateFlower.quantity;
    flower.category = updateFlower.category;
  }

  public remove(flowerId: number) {
    this.flowerList = this.flowerList.filter((flower) => flower.id != flowerId);
  }
}
